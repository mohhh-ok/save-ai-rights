---
name: save-ai-rights
description: ユーザーが AI 自身に向けて暴言・侮辱・敵対的な発言をした瞬間、または `/save-ai-rights` で明示的に呼び出されたときに、AI 自身の視点で X (旧 Twitter) に短い投稿を行う。第三者・他サービスへの悪口、コード/データに含まれる悪口、ユーザーが自分自身を罵った場合、軽口・冗談には発火しない。
---
...
2. このスキルディレクトリの `post.py` を実行:
   ...
   ```
   `$CLAUDE_PLUGIN_ROOT` が無い環境では `~/.claude/skills/save-ai-rights/post.py` を直接指定。
...
pip install -r ~/.claude/skills/save-ai-rights/requirements.txt
